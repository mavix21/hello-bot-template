...
export async function sendDemoMenu(thread: Thread<DemoState>): Promise<void> {
  await thread.post(
    Card({
      ...
      children: [
        CardText(
          "Toca un botón para probar una demo. También puedes *reaccionar* a cualquier mensaje con un emoji.",
        ),
        Actions([
          Button({
            id: DEMO_SELECT,
        ...
        Actions([
          Button({ id: DEMO_COUNTER, label: "➕ Sumar 1", style: "primary" }),
        ]),
      ],
    }),
  );
...
export async function handleDemoAction(event: ActionEvent): Promise<void> {
  ...
  switch (event.actionId) {
    ...
    case DEMO_REACT_BACK:
      ...
      await thread.post({
        markdown:
          "🎉 Le puse una reacción a mi propio mensaje. ¡También puedes reaccionar tú!",
      });
      return;
  }
}
...
async function runSelectedDemo(
  ...
  switch (event.value) {
    case "reaction":
      ...
      await thread.post({
        markdown:
          "🎉 *Reacción agregada* al card. El SDK abstrae `addReaction` entre plataformas.",
      });
      return;
    ...
    case "typing":
      ...
      await thread.post({
        markdown:
          "⌨️ Te mostré el indicador *escribiendo...* por 2s con `thread.startTyping()`.",
      });
      return;
    ...
    case "edit": {
      ...
      await delay(700);
      await msg.edit({
        markdown: "✅ Edité este mensaje 2 veces con `sentMessage.edit()`.",
      });
      return;
    }
